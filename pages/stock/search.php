...
if (isset($_GET['doSearch'])){$doSearch = htmlentities($_GET['doSearch'], ENT_QUOTES, "UTF-8");}
...
$(function() {
    $(".paging").delegate("a", "click", function(event) {	
		...
		$('#sResource').load('/pages/stock/'+url+'&doSearch=resource');
...
<?php
	...
	if($_POST['name'] || $_GET['search']){

        if(isset($_POST['name'])){$s = mysqli_real_escape_string($conn, $_POST['name']);}
        if(isset($_GET['search'])){$s = mysqli_real_escape_string($conn, $_GET['search']);}	
        $search = " WHERE (r.id LIKE '%$s%' || r.name LIKE '%$s%')";
        $searchUrl = 'search='.$s.'&';	
        }else{
        $searchUrl = null;	
        $search = null;	
        ...
        $count_GL7 = mysqli_num_rows($resultGL7) or die (mysqli_error($conn));  //IEGŪST SKAITU 					
            if ($count_GL7!=0){
                ...
                echo '
                <button type="submit" class="btn btn-default btn-sm" id="savebtn"><i class="glyphicon glyphicon-floppy-save" style="color: blue;"></i> saglabāt</button>
