...
<script>
	...
	$(document).ready(function () {
		$('#edit_uom').on('submit', function(e) {
			...
			$.ajax({
				url : '/pages/cargo_setup/post.php?r=editUom',
				type: "POST",
				data: $(this).serializeArray(),
				beforeSend: function(){
				...
				success: function (data) {
					$('#cargoType').load('/pages/cargo_setup/cargo_setup.php?view=edit<?=$elpage;?>&id=<?=$id;?>&res='+data);
				},
				error: function (jXHR, textStatus, errorThrown) {
					alert(errorThrown);
				}
...
if (!$view){
	...
	echo '<div id="cargoType">';									
		echo '<div class="page-header" style="margin-top: -5px;">
			...
			if ($res=='error' ||  $res=='productCodeExists' || $res=='productCodeDoesNotExists' || $res=='uomcodeexists'){echo '<div class="pull-right" style="margin-top: -2px;" id="hideMessage"><div class="btn btn-danger">kļūda!</div></div>';}
			...
			while($rowc = mysqli_fetch_array($resultGL7c)){
				...
				echo '	<tr>
						...
						if($p_edit=='on'){
							...
							if($rowc['status']==0){
								echo '<a class="btn btn-default btn-xs" id="cret'.$rowc['id'].'" onclick="codeRet('.$rowc['id'].')"><i class="glyphicon glyphicon-refresh" style="color: #3BC0DE"  title="atjaunot"></i> atjaunot</a>';
							}
						}	
							echo '
...
if($view=='edit' && $p_edit=='on'){
	...
	echo '<div id="cargoType">';									
		...
		$getDetails = mysqli_query($conn, "SELECT code, NAVcode, name FROM unit_of_measurement WHERE code='".$code."'");
				...
				<select class="form-control selectpicker btn-group-xs" name="NAVuom" id="NAVuom"  data-live-search="true" title="NAV kods">';	
					...
					while($rowi = mysqli_fetch_array($selectuom)){
						echo '<option value="'.$rowi['code'].'"';
							if($rowDet['NAVcode']==$rowi['code']){echo ' selected';}
						echo' >'.$rowi['code']; if($rowi['description']){ echo ' ('.$rowi['description'].')'; } echo '</option>';
					}
