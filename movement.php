...
if (!$view){?>
	...
	$query = "SELECT *,
		(select COUNT(id) from cargo_line WHERE docNr=cargo_header.docNr AND action='23') AS acount
	 FROM cargo_header WHERE (status='20' || rowSent='1') ORDER BY acount DESC, deliveryDate";  //NEPIECIEŠAMAIS VAICĀJUMS
