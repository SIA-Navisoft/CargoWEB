...
$table = mysqli_query($conn, "
    ...
    l.id AS lid, l.docNr AS receive_docNr, l.thisTransport AS receive_transport, l.productNr AS receive_productNr, l.activityDate AS receive_date, h.ladingNr AS receive_ladingNr, l.place_count AS receive_place_count, h.cargo_status AS receive_cargo_status, woi.period_to,
		  
    i.id AS iid, i.docnr AS idocNr, i.deliverydate AS ideliveryDate, i.activitydate AS iactivityDate, i.assistant_amount AS iassistant_amount, 
    i.amount AS iamount, i.productnr AS iproductNr, i.productumo AS iproductUmo, i.status AS istatus, 
...
FROM   web_objects_invoices AS woi 
  ...
  LEFT JOIN item_ledger_entry    AS i  ON e.orgline = i.cargoline 
  LEFT JOIN agreements_lines     AS a  ON h.agreements = a.contractnr AND e.productnr = a.item  AND e.resource = a.service
        JOIN additional_uom       AS au ON e.productnr = au.productnr AND au.status = 1 AND au.convert_from = 1
...
") or die(mysqli_error($conn));
    ...
    while($row = mysqli_fetch_array($table)){
        ...
        if($row['status']==40 || $row['istatus']==40){
            $daysBetweenDatesTotal = daysBetweenDates($deliveryDate, $row['activityDate'])-1;
        }else{
            $daysBetweenDatesTotal = daysBetweenDates($deliveryDate, $row['period_to'])-1;
        }       
